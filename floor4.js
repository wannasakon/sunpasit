...
const getTasks = () => db.collection("floor4").get();

const onGetTasks = (callback) => db.collection("floor4").onSnapshot(callback);

const deleteTask = (id) => db.collection("floor4").doc(id).delete();

const getTask = (id) => db.collection("floor4").doc(id).get();

const updateTask = (id, updatedTask) => db.collection('floor4').doc(id).update(updatedTask);
...
taskForm.addEventListener("submit", async (e) => {
  ...
  try {
    if (!editStatus) {
      await saveTask(name.value,iconLocation.value,imageLocation1.value,imageLocation2.value,route1.value,route2.value,status.value);
    } else {
      await updateTask(id, {
        name: name.value,
